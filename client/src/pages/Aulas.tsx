...
import {
  ...
  Music, LogIn, ChevronDown, ChevronUp, FileText, ExternalLink, MessageCircle,
} from "lucide-react";
...
function ComentariosAdmin({ aulaId }: { aulaId: number }) {
  const utils = trpc.useUtils();
  const { data: comentarios = [] } = trpc.comentarios.list.useQuery({ tipo: "aula", referenciaId: aulaId });
  const deleteComentario = trpc.comentarios.delete.useMutation({
    onSuccess: () => utils.comentarios.list.invalidate({ tipo: "aula", referenciaId: aulaId }),
  });
  const responderComentario = trpc.comentarios.responder.useMutation({
    onSuccess: () => utils.comentarios.list.invalidate({ tipo: "aula", referenciaId: aulaId }),
  });
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto">
        {(comentarios as any[]).map((c: any) => (
          <div key={c.id} className="bg-muted/40 rounded-xl p-3 space-y-2">
            {/* Comentário do aluno */}
            <div className="flex items-start gap-2">
              ...
              <div className="flex gap-1 flex-shrink-0">
                {!c.resposta && respondendo !== c.id && (
                  <Button size="sm" variant="outline"
                    onClick={() => { setRespondendo(c.id); setTextoResposta(""); }}
            ...
            {c.resposta && (
              <div className="ml-3 pl-3 border-l-2 border-primary/30 space-y-0.5">
                <p className="text-[10px] font-semibold text-primary uppercase tracking-wide">Catequista respondeu</p>
                <p className="text-sm text-foreground leading-relaxed">{c.resposta}</p>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-muted-foreground">
                    {c.respondidoEm ? new Date(c.respondidoEm).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" }) : ""}
                  </span>
                  <button
                    onClick={() => { setRespondendo(c.id); setTextoResposta(c.resposta); }}
            ...
            {respondendo === c.id && (
              <div className="ml-3 space-y-1.5">
                <textarea
                  ...
                  rows={2}
                  className="w-full text-xs border border-primary/30 rounded-lg px-3 py-2 bg-background focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none"
                  autoFocus
                />
                <div className="flex gap-1.5">
                  <Button size="sm"
                    onClick={() => {
                      ...
                      responderComentario.mutate({ id: c.id, resposta: textoResposta.trim() },
                        { onSuccess: () => { setRespondendo(null); setTextoResposta(""); } });
                    }}
                    disabled={!textoResposta.trim() || responderComentario.isPending}
                    className="flex-1 h-7 text-xs bg-primary text-primary-foreground">
                    {responderComentario.isPending ? "Enviando..." : <><Check className="w-3 h-3 mr-1" /> Enviar resposta</>}
                  </Button>
                  <Button size="sm" variant="outline"
        ...
        return (
          <Card key={aula.numero} className={cn(
            ...
            {isExpanded && (
              ...
              <ComentariosAdmin aulaId={aula.id} />
